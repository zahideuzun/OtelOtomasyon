	...
	public partial class FrmRapor : Form
	{
		...
		private void btnAra_Click(object sender, EventArgs e)
		{
			if (KontrolYapısıExtensionMethod())
			{
				...
				var sorgu = _odemeAlinanMusteriListesi.Where(x => x.Musteri.AdSoyad == tbMusteriAra.Text).ToList();
		...
		{
			foreach (var item in _odemeAlinanMusteriListesi)
			{
				if (numTarih1.Value <= item.GirisTarihi && numTarih1.Value >= item.CikisTarihi)
				{
					int sayac = 1;
					ListViewItem lv = new ListViewItem(item.Musteri.AdSoyad, 0);
					lv.Text = (sayac++).ToString();
